...
class DayTraderAgent(AgentBase):
    ...
    # ---------------- mechanical (no-LLM) autopilot ----------------

    def run_mechanical(self) -> RunOutcome:
        """Deterministic, LLM-free trading tick — the UNATTENDED loop body.

        Applies the same rules the LLM would (rank by heat_score, require news +
        analyst agreement with the direction, enforce every cap/stop) but with a
        fixed policy instead of a model call — so it keeps trading with no human
        and no PAT. Runs on the scheduler's own connection/cadence.
        """
        start = time_ms()
        if self._kill_switched():
            rid = self._record_run(status="halted", prompt="mechanical", response=None,
                                    tools_called=None, decisions=None,
                                    error="kill_switch", latency_ms=time_ms() - start)
            return RunOutcome(status="halted", error="kill_switch", run_id=rid)
        wall = self._wall()

        # Daily drawdown halt (same guard as the LLM path).
        starting_equity = self._starting_equity_today(wall)
        acct = self.broker.get_account(self.sub_account)
        if starting_equity > 0:
            dd_pct = 100.0 * (acct.equity - starting_equity) / starting_equity
            if dd_pct <= DD_HALT_PCT:
                rid = self._record_run(status="halted", prompt="mechanical", response=None,
                                        tools_called=None,
                                        decisions=[{"halt": "daily_drawdown", "dd_pct": dd_pct}],
                                        error=f"drawdown:{dd_pct:.2f}%",
                                        latency_ms=time_ms() - start)
                return RunOutcome(status="halted", error="daily_drawdown", run_id=rid)

        # End-of-day: flatten and stop.
        if is_force_flat_window(wall):
            decisions, orders = self._force_flat()
            rid = self._record_run(status="ok", prompt="force_flat", response=None,
                                    tools_called=None, decisions=decisions,
        ...
        # 1) Protect the book (stops/targets) first.
        stop_actions = self._manage_open_positions()

        # 2) Pick trades by rule (no new entries in the last 30 min of the session).
        proposals: list[_Proposal] = []
        exits: list[dict] = []
        if not is_force_flat_window(wall, minutes_before_close=30):
            proposals, exits = self._mechanical_pick(acct)

        # 3) Size + place through the SAME engine as the LLM path.
        proposals = self._maybe_substitute_inverse(proposals)
        proposals = self._reprice_to_market(proposals)
        decisions = self._validate_and_size(proposals)
        orders = self._place(decisions)
        ...
        if stop_actions:
            all_decisions = [{"stop_exit": a} for a in stop_actions] + all_decisions
        placed = [d for d in decisions if d.accepted]
        summary = ("mechanical: " +
                   (", ".join(f"{d.side} {d.symbol}" for d in placed) if placed
                    else ("exits: " + ", ".join(e["symbol"] for e in exits) if exits
                          else "no qualified setup")))
        rid = self._record_run(status="ok", prompt="mechanical", response=summary,
                                tools_called=None, decisions=all_decisions,
                                error=None, latency_ms=time_ms() - start)
        return RunOutcome(status="ok", decisions=all_decisions, orders=orders, run_id=rid)

    def _mechanical_pick(self, acct) -> tuple[list[_Proposal], list[dict]]:
        """Rule-based selection for run_mechanical: manage the book, then pick at
        most one qualifying new entry. Returns (proposals, exits)."""
        from src.config import get_settings
        s = get_settings()
        heat_min = float(getattr(s, "day_mechanical_heat_min", 60.0) or 0.0)

        positions = self.broker.list_positions(self.sub_account)
        equity = acct.equity or 0.0
        dust = equity * DUST_POSITION_PCT if equity else 0.0
        material = [p for p in positions if abs(p.qty) * (p.mark_price or 0.0) >= dust]
        held = {p.symbol.upper() for p in positions if abs(p.qty) > 1e-9}
        cooldown = self._recent_traded_symbols()
        book_full = len(material) >= MAX_CONCURRENT_POSITIONS

        exits: list[dict] = []
        # If the book is full, free a slot by exiting the single weakest LOSER
        # (most negative unrealized P&L). Next tick then has room to rotate in.
        if book_full:
            losers = sorted(material, key=lambda p: getattr(p, "unrealized_pnl", 0.0) or 0.0)
            for p in losers:
                sym = p.symbol.upper()
                if _is_option_symbol(sym):
                    continue
                if (getattr(p, "unrealized_pnl", 0.0) or 0.0) >= 0:
                    break  # no losers to cut; leave winners alone
                try:
                    ...
                    for pl in dbm.get_active_position_plans(self.conn, aid):
                        if str(pl["symbol"]).upper() == sym:
                            dbm.close_position_plan(self.conn, pl["id"], "mechanical_rotate")
                    exits.append({"symbol": sym, "reason": "mechanical: weakest loser, freeing slot",
                                   "status": getattr(res, "status", None)})
                except Exception:
                    log.exception("mechanical: failed to exit %s", sym)
                break
            return [], exits  # rotate this tick; enter next tick with the free slot

        # Room available: pick the best qualifying idea (one per tick).
        try:
            res = self.short_term.list_ideas(mode="intraday", tier="A", limit=15)
        except Exception:
            log.debug("mechanical: list_ideas failed", exc_info=True)
            return [], exits
        rows = (res.get("rows") or res.get("ideas") or []) if isinstance(res, dict) else []
        for r in rows:
            sym = str(r.get("ticker") or r.get("symbol") or "").upper()
            if not sym or sym in held or sym in cooldown:
                continue
            direction = str(r.get("direction") or "").lower()
            side = "sell" if direction == "short" else "buy"
            # skip if the effective (post-inverse) symbol is already held/cooled
            eff = INVERSE_ETF.get(sym, sym) if side == "sell" else sym
            if eff in held or eff in cooldown:
                continue
            heat = float(r.get("heat_score") or 0.0)
            if heat < heat_min:
                continue
            entry, stop = r.get("entry"), r.get("stop")
            if not entry or not stop:
                continue
            # Diligence: news sentiment must agree with the direction and the
            # analyst rating must not strongly contradict it.
            news = self._news(sym, days=2)
            ss = news.get("sentiment_score")
            analyst = self._analyst_view(sym)
            rating = str(analyst.get("rating") or "")
            if side == "buy":
                if ss is not None and ss < -0.05:
                    continue
                ...
                if rating in ("Buy", "Strong Buy"):
                    continue
            thesis = (f"mechanical: heat {heat:.0f}, news {news.get('sentiment_label') or 'n/a'}, "
                      f"analyst {rating or 'n/a'}")
            return [_Proposal(symbol=sym, entry_price=float(entry), stop_price=float(stop),
                              side=side, thesis=thesis)], exits
        return [], exits
    ...
    def manage_positions_only(self) -> list[dict]:
        ...
        In the end-of-day force-flat window it flattens the whole book instead —
        so positions are always closed by ~15:55 ET even in Copilot-driven mode,
        where the LLM `run_once` (which normally owns force-flat) never runs.
        """
        if self._kill_switched() or not is_market_open(self._wall()):
            return []
        if is_force_flat_window(self._wall()):
            decisions, orders = self._force_flat()
            if decisions:
                self._record_run(status="ok", prompt="force_flat", response=None,
                                 tools_called=None, decisions=decisions,
                                 error=None, latency_ms=0)
            return [{"force_flat": d} for d in decisions]
